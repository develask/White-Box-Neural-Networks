...
for i in range(16):
	...
	raw = dnn.Input(7, "raw_s_"+str(i+1))
	h1 = dnn.Fully_Connected(7, "sigmoid", "r1_"+str(i+1))
	h2 = dnn.Fully_Connected(7, "sigmoid", "r2_"+str(i+1))
...
raws = [dnn.Input(7, "raw "+str(i+1)) for i in range(16)]

# h1s = [dnn.Fully_Connected(28, "sigmoid", "h1_"+str(i+1)) for i in range(16)]
# h2s = [dnn.Fully_Connected(16, "sigmoid", "h2_"+str(i+1)) for i in range(16)]

h3 = dnn.Fully_Connected(64, "sigmoid", "h3")

ou = dnn.Fully_Connected(10, "softmax", "output")
...
print("data training")
start = time.clock()
nn.SGD(training_data[int(len_data*0.01):], training_data[:int(len_data*0.01)], 128, 4, 0.1, 0.05)
end = time.clock()
